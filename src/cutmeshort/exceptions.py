...
def _truncate_value(value: Any, max_length: int = 1024) -> Any:
    if value is None:
        return None
    text = str(value)
...
class OpenApiException(Exception):
    """The base exception class for all CMS exceptions"""
...
class ApiException(OpenApiException):
    ...
    def __str__(self):
        ...
        if self.body:
            error_message += "HTTP response body: {0}\n".format(_truncate_value(self.body))

        if self.data:
            error_message += "HTTP response data: {0}\n".format(_truncate_value(self.data))
